...
df_wnd_top_ticket = df_wnd_ticket.loc[(df_wnd_ticket['признак_выходного']>170)]
...
df_act_top_ticket = df_act_ticket.loc[(df_act_ticket['признак_выходного']>2500)]
...
df_act_top_date.plot(ax=axes[0,0],figsize=(14,10), grid='on', title='Топ часов в рабочие дни',                     color='red', label='line 1', linewidth=4, linestyle =':')
df_wnd_top_date.plot(ax=axes[0,1],figsize=(14,10), grid='on', title='Топ часов в выходные',                     color='black', label='line 1', linewidth=3, linestyle ='--')

df_act_top_ticket.plot(ax=axes[1,0],figsize=(14,10), grid='on', title='Наиболее популярные способы оплаты в рабочие',                       color='black', label='line 1', linewidth=4, linestyle =':')
df_wnd_top_ticket.plot(ax=axes[1,1],figsize=(14,10), grid='on', title='Наиболее популярные способы оплаты в выходные',                       color='red', label='line 1', linewidth=3, linestyle ='--')

plt.savefig(r'C:\Users\Zodiac\Desktop\Саше\График.png')
...
df_wnd['признак_выходного'].count()


# In[30]:


df_act['признак_выходного'].count()


# In[31]:


max_ride = df_act_top_date.max()

avg_difference = df_act['признак_выходного'].count()/5 - df_wnd['признак_выходного'].count()/2
#"Запас прочности по часам" + разница средних нагрузок по часам
print(f'{df_act_top_date-df_wnd_top_date}' + '/////' + f'{avg_difference}')
